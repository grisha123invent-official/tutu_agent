...
## Локальные модели (self-hosted, без облака)

Ассистент умеет работать **полностью на открытых локальных моделях** — без единого
платного сервиса. Все интеграции сделаны через **OpenAI-совместимые API**, поэтому
переключение — только переменными в `backend/.env`, без правки кода.

```mermaid
flowchart LR
  subgraph Browser["Браузер (фронт не меняется)"]
    UI["UI + чат + голос"]
  end

  subgraph Backend["Бэкенд (Node) — слой провайдеров"]
    Agent["Агент: tool-calling + MCP"]
    VoiceSw{{"VOICE_PROVIDER"}}
    LlmSw{{"LLM_BASE_URL"}}
    WebSw{{"WEB_SEARCH_PROVIDER"}}
  end

  subgraph Cloud["Облако (прод сейчас)"]
    OAI["OpenAI gpt-4o"]
    RT["OpenAI Realtime"]
    OWS["OpenAI web_search"]
  end

  subgraph Local["Локально (self-hosted, бесплатно)"]
    OLL["Ollama / vLLM\nQwen2.5-Instruct"]
    LRT["Локальный Realtime\n(HF speech-to-speech / LocalAI)"]
    PIPE["Конвейер STT→LLM→TTS\n(Whisper + Piper/Kokoro)"]
    SX["SearXNG"]
  end

  MCP["MCP Туту (не меняется)"]

  UI <--> Agent
  Agent --> MCP
  LlmSw -->|"пусто"| OAI
  LlmSw -->|"локальный URL"| OLL
  VoiceSw -->|"openai_realtime"| RT
  VoiceSw -->|"openai_realtime + REALTIME_URL"| LRT
  VoiceSw -->|"local"| PIPE
  WebSw -->|"openai"| OWS
  WebSw -->|"searxng"| SX
```

| Часть | Облако (прод) | Локальная замена (open-source) |
|---|---|---|
| LLM + вызов MCP | OpenAI gpt-4o | Qwen2.5-Instruct / Llama 3.x через Ollama или vLLM |
| Голос | OpenAI Realtime | локальный Realtime-сервер **или** конвейер Whisper → LLM → Piper/Kokoro |
| Веб-поиск | OpenAI web_search | SearXNG (свой) |
| Поиск билетов | MCP Туту | MCP Туту (внешний сервис, без изменений) |

Готовый конфиг: `backend/.env.local-models.example` · модель-серверы:
`infra/local-models/docker-compose.yml` · **подробный гайд (модели, железо, шаги):

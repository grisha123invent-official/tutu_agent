...
## 2. Два пути для локального голоса

Есть **два способа** сделать голос без облака — выбирайте по потребности:

| | (A) Локальный Realtime | (B) Конвейер STT→LLM→TTS |
|---|---|---|
| Что это | готовый сервер, совместимый с OpenAI Realtime API | наша сборка из отдельных STT/LLM/TTS |
| Как включить | `REALTIME_URL=ws://…/v1/realtime` | `VOICE_PROVIDER=local` |
| Правка кода | нет (тот же `RealtimeBridge`, меняется URL) | уже реализовано (`voice/localVoice.ts`) |
| Задержка/дуплекс | ниже, естественнее (стриминг, перебивание) | выше (пауза → распознал → ответил) |
| Поиск билетов (MCP) | **зависит от сервера** — нужен tool calling в его realtime | **да, точно** (агент вызывает MCP) |

**Рекомендация под наш кейс:** голос должен уметь искать билеты через MCP,
поэтому надёжный вариант — **(B) конвейер** (tool calling гарантирован). Вариант
**(A)** берите, если ваш локальный Realtime-сервер поддерживает function calling
(см. раздел 9), либо если голосу не нужен поиск (просто разговор).

Оба пути включаются только через `backend/.env` — фронтенд не меняется.

## 2.1. Архитектура конвейера STT→LLM→TTS (вариант B)

Готовой локальной модели «речь→речь» с вызовом инструментов нет, поэтому голос
собирается конвейером (реализован в `backend/src/voice/localVoice.ts`):
...
## 9. Локальный Realtime (голос↔голос, как у OpenAI) — вариант A

Наш `RealtimeBridge` говорит по протоколу **OpenAI Realtime API**. Поэтому его
можно направить на **локальный сервер, совместимый с этим протоколом** — одной
переменной, без изменения кода:

```env
VOICE_PROVIDER=openai_realtime          # оставляем realtime-мост
REALTIME_URL=ws://localhost:8765/v1/realtime   # адрес локального сервера
REALTIME_MODEL=<модель на сервере>
REALTIME_VOICE=<голос>
# REALTIME_API_KEY=  # локальные обычно без ключа
```

### Готовые продукты (open-source, self-hosted, бесплатно)

| Продукт | Что даёт | Tool calling (поиск MCP) | Ссылка |
|---|---|---|---|
| **HuggingFace speech-to-speech** | OpenAI-Realtime-совместимый сервер `ws://…/v1/realtime`, сменные STT/LLM/TTS | зависит от связки (LLM-слот OpenAI-совместимый) | github.com/huggingface/speech-to-speech |
| **LocalAI** | Realtime API поверх пайплайна VAD+STT+LLM+TTS | частично (проверять) | localai.io/features/openai-realtime |
| **Speaches** | OpenAI-совместимые STT/TTS + `/v1/realtime` | частично (проверять) | github.com/speaches-ai/speaches |
| **Kyutai Unmute** | лучший дуплекс/латентность (~450мс), семантический VAD, оборачивает ЛЮБОЙ LLM | **нет** (в разработке) — голос без поиска MCP | github.com/kyutai-labs/unmute |

### Важно про наш кейс

- Голос у нас должен **заполнять форму и искать билеты через MCP** — это делается
  через **function/tool calling** в realtime-сессии. Не все локальные realtime-серверы
  это поддерживают. Если поддерживает (проверьте на своей связке) — вариант A
  идеален: минимальная задержка + перебивание. Если нет — берите **вариант B**
  (конвейер), он гарантированно вызывает MCP.
- **Kyutai Unmute** — самый приятный по качеству/латентности разговор, но **tool
  calling пока не поддерживает** (официально в TODO). Подойдёт для «просто
  поговорить», не для голосового поиска билетов. Требует GPU ≥16 ГБ VRAM, Linux, x86_64.

### Как выбрать

```
Нужен голосовой ПОИСК билетов (MCP)?
├─ Да  → сервер realtime с tool calling  ИЛИ  вариант B (конвейер, надёжно)
└─ Нет (просто разговор) → Kyutai Unmute (лучший UX)  ИЛИ любой realtime-сервер
```

### Матрица провайдеров голоса (итог)

| `VOICE_PROVIDER` | `REALTIME_URL` | Что работает |
|---|---|---|
| `openai_realtime` | не задан | облачный OpenAI Realtime (прод) |
| `openai_realtime` | задан (локальный) | локальный Realtime-сервер (вариант A) |
| `local` | — | конвейер STT→LLM→TTS (вариант B) |
